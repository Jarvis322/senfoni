...
function HeroSectionForm({ initialData }: { initialData: HeroSection }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await updateLayoutSection('heroSection', formData);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">Ana Banner (Hero) Düzenleme</h1>
            <Link href="/admin/layout" className="text-blue-600 hover:text-blue-800">
              Geri Dön
            </Link>
          </div>
          <p className="mt-1 text-sm text-gray-600">
            Ana sayfa banner bölümünü buradan düzenleyebilirsiniz.
          </p>
        </div>
